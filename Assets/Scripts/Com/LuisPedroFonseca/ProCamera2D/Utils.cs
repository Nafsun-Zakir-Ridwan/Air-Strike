...
namespace Com.LuisPedroFonseca.ProCamera2D
{
	public static class Utils
	{
		...
		public static float SmoothApproach(float pastPosition, float pastTargetPosition, float targetPosition, float speed, float deltaTime)
		{
			...
			return targetPosition - num2 + num3 * Mathf.Exp(-num);
		}
		...
		private static void DrawArrowEnd(bool gizmos, Vector3 pos, Vector3 direction, Color color, float arrowHeadLength = 0.25f, float arrowHeadAngle = 20f)
		{
			...
			Vector3 a2 = Quaternion.LookRotation(direction) * Quaternion.Euler(-arrowHeadAngle, 0f, 0f) * Vector3.back;
			Vector3 a3 = Quaternion.LookRotation(direction) * Quaternion.Euler(0f, arrowHeadAngle, 0f) * Vector3.back;
			Vector3 a4 = Quaternion.LookRotation(direction) * Quaternion.Euler(0f, -arrowHeadAngle, 0f) * Vector3.back;
			if (gizmos)
			{
				Gizmos.color = color;
		...
		public static Vector2 GetScreenSizeInWorldCoords(Camera gameCamera, float distance = 10f)
		{
			...
			{
				num = 2f * Mathf.Abs(distance) * Mathf.Tan(gameCamera.fieldOfView * 0.5f * 0.0174532924f);
				x = num * gameCamera.aspect;
			}
		...
		{
			return pointX >= x - width * 0.5f && pointX <= x + width * 0.5f && pointY >= y - height * 0.5f && pointY <= y + height * 0.5f;
		}
